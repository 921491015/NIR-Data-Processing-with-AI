...
data_x = x
data_y = y
y_pls=pd.get_dummies(y)

'''pls处理过程'''
plsmod = PLSRegression(n_components=83)
plsmod.fit(x, y)
x_test = x
pls_x=plsmod.x_scores_

x_data = np.array(data_x)
print(x_data.shape)
x_data=np.array(pls_x)
print(x_data.shape)
...
NIR = NIR_CONV3(1, 32, 20, 1, 0).to(device)
        ...
        with open(test_result_path, "w") as f2:
            for epoch in range(EPOCH):
                ...
                for i, data in enumerate(train_loader):  # gives batch data, normalize x when iterate train_loader
                    ...
                    print("epoch = {:} Loss = {:.4f}  Acc= {:.4f}".format((epoch + 1), (loss.item()),
